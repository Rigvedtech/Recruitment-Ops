...
class InterviewScheduledStatusEnum(enum.Enum):
    scheduled = "scheduled"
    rescheduled = "rescheduled"

class InterviewScheduled(db.Model):
    __tablename__ = 'interview_scheduled'
    __table_args__ = (
        db.UniqueConstraint('requirement_id', 'profile_id', name='uq_interview_scheduled_requirement_profile'),
    )
    
    interview_scheduled_id = db.Column(GUID, primary_key=True, server_default=postgresql_uuid_default())
    ...
    status = db.Column(db.Enum(InterviewScheduledStatusEnum), nullable=False)
    ...
    def __repr__(self):
        return f'<InterviewScheduled {self.interview_scheduled_id}>'
    
    def to_dict(self):
        return {
            'interview_scheduled_id': str(self.interview_scheduled_id) if self.interview_scheduled_id else None,
