...
class InterviewRoundOne(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('requirement_id', 'profile_id', name='uq_interview_round_one_requirement_profile'),
    )
    
    interview_round_one_id = db.Column(GUID, primary_key=True, server_default=postgresql_uuid_default())
    ...
    def __repr__(self):
        return f'<InterviewRoundOne {self.interview_round_one_id}>'
    
    def to_dict(self):
        return {
            'interview_round_one_id': str(self.interview_round_one_id) if self.interview_round_one_id else None,
